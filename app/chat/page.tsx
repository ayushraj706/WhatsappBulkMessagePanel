...
export default function UltimateCleanChat() {
  ...
  const [lightboxImage, setLightboxImage] = useState<string | null>(null); // For custom field view
  ...
  return (
    <div className="flex h-full w-full bg-white dark:bg-[#050505] text-black dark:text-white overflow-hidden">
            ...
            <div className="pb-8 pt-2 px-4 bg-white dark:bg-[#050505] border-t border-zinc-200 dark:border-zinc-900 shrink-0 z-40 w-full transition-all">
              ...
              <div className="flex items-center gap-2">
                ...
                {isRecording ? (
                  ...
                  <>
                    <button onMouseDown={(e) => { e.preventDefault(); setShowIcons(!showIcons); }} className={cn("transition-all duration-300 shrink-0 p-2 flex items-center justify-center", !showIcons && !previewUrl ? "w-10 opacity-100" : "w-0 opacity-0 overflow-hidden")}><ChevronRight className="w-7 h-7 text-blue-500" /></button>
      ...
      {/* CUSTOM LIGHTBOX (The "Field View") */}
      {lightboxImage && (
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-xl flex flex-col items-center justify-center p-4 animate-in fade-in duration-300">
           <button onClick={() => setLightboxImage(null)} className="absolute top-8 right-8 text-white hover:bg-white/10 p-3 rounded-full transition-colors"><X className="w-10 h-10" /></button>
           ...
           <p className="mt-6 text-white font-bold tracking-widest uppercase text-xs opacity-50">BaseKey Internal Viewer</p>
        </div>
      )}
    </div>
  );
