...
export default function ProMediaChat() {
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setShowIcons(false); // Pill ko lamba karne ke liye icons hide
    }
  };
  ...
  const handleSend = async () => {
    ...
    try {
      // 1. Agar file hai toh upload karo
      if (selectedFile) {
  ...
  return (
    <div className="flex h-full w-full max-w-full bg-white dark:bg-[#050505] text-black dark:text-white overflow-hidden transition-colors duration-500">
      ...
      <div className={cn(
        ...
        <div className="p-4 border-b border-zinc-200 dark:border-zinc-900 font-bold">Messages</div>
        <div className="flex-1 overflow-y-auto">
          {Array.from(new Set(messages.map(m => m.sender))).filter(s => s !== "Me").map((num) => (
            <div key={num} onClick={() => setSelectedContact(num)} className="p-4 flex items-center gap-3 cursor-pointer border-b border-zinc-100 dark:border-zinc-900/50 hover:bg-zinc-50 dark:hover:bg-zinc-900">
      ...
      <div className={cn("flex-1 flex flex-col bg-white dark:bg-[#050505] relative w-full", !selectedContact ? "hidden md:flex" : "flex")}>
        {selectedContact ? (
          <>
            ...
            <div className="flex-1 overflow-y-auto w-full bg-white dark:bg-[#050505]">
              <ChatMessageList className="p-4 pr-6 space-y-6 min-h-full pb-20">
                {messages.filter(m => (m.sender === selectedContact) || (m.type === "sent" && m.receiver === selectedContact)).map((msg) => (
                  <ChatBubble key={msg.id} variant={msg.type === "sent" ? "sent" : "received"}>
                    <ChatBubbleMessage className={cn(
                      ...
                      {msg.imageUrl && (
                        <img src={msg.imageUrl} alt="media" className="rounded-lg max-w-[200px] h-auto mb-2" onClick={() => window.open(msg.imageUrl, '_blank')} />
                      )}
            ...
            <div className="pb-8 pt-2 pl-2 pr-4 bg-white dark:bg-[#050505] border-t border-zinc-200 dark:border-zinc-900 shrink-0 z-40 w-full">
              ...
              {/* MEDIA PREVIEW: Selection ke baad dikhega */}
              {previewUrl && (
                <div className="flex items-center gap-2 mb-3 ml-2 relative w-fit">
                   <img src={previewUrl} alt="preview" className="w-16 h-16 rounded-xl object-cover border-2 border-blue-500 shadow-lg" />
                   <button onClick={cancelSelection} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1"><X className="w-3 h-3"/></button>
                </div>
              )}
              ...
              <div className="flex items-center gap-1">
                <div className={cn(
                  ...
                  {/* Camera Icon - capture="environment" for live */}
                  <button onClick={() => camRef.current?.click()} className="p-2 hover:bg-zinc-100 dark:hover:bg-zinc-900 rounded-full transition"><Camera className="w-6 h-6" /></button>
                  <button onClick={() => galleryRef.current?.click()} className="p-2 hover:bg-zinc-100 dark:hover:bg-zinc-900 rounded-full transition"><ImageIcon className="w-6 h-6" /></button>
                  <button onClick={() => fileRef.current?.click()} className="p-2 hover:bg-zinc-100 dark:hover:bg-zinc-900 rounded-full transition"><Paperclip className="w-6 h-6 rotate-45" /></button>
                  <button className="p-2 hover:bg-zinc-100 dark:hover:bg-zinc-900 rounded-full transition"><Mic className="w-6 h-6" /></button>
                </div>
                ...
                <div className="flex items-center min-w-[44px] justify-center shrink-0">
                  {/* SEND TOGGLE: Text ya Selection par Send button */}
                  {(inputText.trim().length > 0 || selectedFile) ? (
                    <button 
                      onClick={handleSend} 
                      disabled={isUploading}
                      className={cn("p-2.5 bg-blue-600 rounded-full shadow-lg transition-all", isUploading && "opacity-50 animate-pulse")}
                    >
                      <Send className="w-5 h-5 text-white fill-current" />
                    </button>
                  ) : (
                    <button onClick={() => handleSend("❤️")} className="p-2 transition-all text-black dark:text-red-500"><MessengerHeart className="w-7 h-7" /></button>
                  )}
                </div>
              </div>
            </div>
          ...
          <div className="flex-1 flex items-center justify-center text-zinc-400 opacity-40">BaseKey Panel</div>
        )}
      </div>
    </div>
  );
