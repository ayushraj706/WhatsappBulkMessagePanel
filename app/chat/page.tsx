...
export default function CleanMessengerChat() {
  ...
  // SEND LOGIC: Optimistic UI + Meta ID Tracking
  const handleSend = async (customText?: string) => {
    ...
    try {
      // 1. Firebase mein turant dikhao (Sending Status)
      ...
      // 3. Status Update (Meta ID save karo for real ticks)
              ...
              <ChatMessageList className="p-4 space-y-4 pb-24">
                {messages.filter(m => (m.sender === selectedContact) || (m.type === "sent" && m.receiver === selectedContact)).map((msg) => (
                  <ChatBubble key={msg.id} variant={msg.type === "sent" ? "sent" : "received"}>
                    {/* BORDERLESS BUBBLE: Added 'p-1' for media */}
                    <ChatBubbleMessage className={cn(
                      ...
                      {msg.imageUrl && (
                        <div className={cn("overflow-hidden rounded-xl", msg.status === "sending" && "opacity-40 grayscale")}>
                           <img src={msg.imageUrl} alt="media" className="max-w-[260px] h-auto cursor-pointer" onClick={() => window.open(msg.imageUrl, '_blank')} />
                        </div>
                      )}
                      {msg.audioUrl && (
                        <div className="p-2 min-w-[200px]">
                           <audio controls className="w-full h-8 invert dark:invert-0"><source src={msg.audioUrl} /></audio>
                           <p className="text-[10px] mt-1 opacity-70">Voice Note</p>
                        </div>
                      )}
                      ...
                      {msg.type === "sent" && (
                        <div className="flex justify-end mt-0.5 -mr-1">
                          ...
                          {msg.status === "sent" && <Check className="w-3.5 h-3.5 text-red-500" />}
                          {msg.status === "delivered" && <CheckCheck className="w-3.5 h-3.5 text-red-400" />}
                          {msg.status === "read" && <CheckCheck className="w-3.5 h-3.5 text-red-600 font-bold" />}
                        </div>
                      )}
                    </ChatBubbleMessage>
                  </ChatBubble>
            ...
            <div className="pb-8 pt-2 px-4 bg-white dark:bg-[#050505] border-t border-zinc-200 dark:border-zinc-900 shrink-0 z-40 w-full transition-all">
              {previewUrl && (
                <div className="flex items-center gap-2 mb-3 relative w-fit scale-in-center">
                   ...
                   <button onClick={() => {setSelectedFile(null); setPreviewUrl(null); setShowIcons(true);}} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1"><X className="w-3 h-3"/></button>
                </div>
              )}
              ...
              <div className="flex items-center gap-2">
                {!isRecording && (
                  <div className={cn("flex items-center transition-all", showIcons ? "w-auto opacity-100" : "w-0 opacity-0 invisible overflow-hidden")}>
                    <button onClick={() => camRef.current?.click()} className="p-2.5 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-full transition"><Camera className="w-6 h-6" /></button>
                    <button onClick={() => galleryRef.current?.click()} className="p-2.5 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-full transition"><ImageIcon className="w-6 h-6" /></button>
                  </div>
                )}
                ...
                {isRecording ? (
                  <div className="flex-1 flex items-center justify-between bg-red-50 dark:bg-red-950/20 px-4 py-2 rounded-full border border-red-200 dark:border-red-900/50">
                    ...
                    <button onClick={stopRecording} className="text-red-600 font-bold text-xs uppercase"><Square className="w-4 h-4 inline mr-1" /> STOP</button>
                  </div>
                ) : (
                  <div className="flex-1 flex items-center bg-zinc-100 dark:bg-zinc-900 rounded-[28px] px-4 py-1.5 focus-within:ring-1 ring-zinc-300 dark:ring-zinc-800 transition-all">
                    ...
                    <button className="text-zinc-500 p-1.5 hover:scale-110"><MessengerSmile /></button>
                  </div>
                )}
